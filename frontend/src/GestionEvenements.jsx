...
function GestionEvenements() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editingEvent) {
        response = await axios.put(`http://localhost:5000/api/evenements/${editingEvent.ID_evenement}`, formDataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        response = await axios.post(`http://localhost:5000/api/evenements`, formDataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:5000/api/evenements/${id}`);
