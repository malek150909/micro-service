...
const GestionEnseignants = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      // Filter out skipped grades (students not found) before setting the grades state
      ...
      // Display the editable table if there are valid grades
  ...
  const handleSubmitGrades = async () => {
    ...
    try {
      const res = await api.post('/grades/submit', {
  ...
  return (
    ...
    <div className="container">
      <nav>
        <button onClick={() => navigate('/notes')}>Retour</button>
      ...
      <div className="upload-section">
        <h3>
          Importer via Excel <FontAwesomeIcon icon={faUpload} style={{ verticalAlign: 'middle', color: '#1565c0' }} />
        </h3>
        <input type="file" accept=".xlsx" onChange={handleFileUpload} disabled={!selectedModule || !selectedSemestre} />
      </div>
      ...
      {showExistingGrades && (
        <>
          ...
          {existingGrades.length > 0 ? (
            ...
            <p>Aucune note existante pour ce module, cette section et ce semestre.</p>
          )}
        </>
      )}
    </div>
