...
function Home() {
    ...
    const fetchResources = async () => {
        try {
            ...
            const response = await fetch('http://courses.localhost/ressources/resources', {
                headers: {
    ...
    const handleDeleteResource = async (id) => {
        try {
            ...
            const response = await fetch(`http://courses.localhost/ressources/resources/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'matricule': matricule
...
const ResourceModal = ({ resource, onClose, onUpdate, matricule }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://courses.localhost/ressources/resources/${resource.ID_ressource}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'matricule': matricule
                    ...
                    {!isEditing ? (
                        <div className={styles['ENS-RES-modal-view']}>
                            ...
                            <div className={styles['ENS-RES-modal-field']}>
                                ...
                                <a href={`http://courses.localhost${resource.fichier_url}`} target="_blank" rel="noopener noreferrer">
