...
const getCalendarEvents = async (req, res) => {
  ...
  try {
    ...
    events = [...calendarEvents.map(event => ({
      ...event,
      type: event.title === 'Événements administratifs' ? 'administratif' : 'personal',
      event_date: format(new Date(event.event_date), 'yyyy-MM-dd'),
      canDelete: true,
      canEdit: event.title !== 'Événements administratifs'
    }))];
    ...
    if (isTeacher) {
      ...
      events = [...events, ...suppSessions.map(session => ({
        ...session,
        type: 'supp_session',
        title: `Séance supplémentaire (${session.nom_module})`,
...
const addPersonalEvent = async (req, res) => {
  ...
  try {
    ...
    const isPastDate = event_date < today;
    const pastDateConfirmed = req.headers['x-past-date-confirmed'] === 'true';

    if (isPastDate && !pastDateConfirmed) {
      return res.status(400).json({ message: 'Cette date est passée, confirmez pour ajouter l’événement.' });
    }
...
const updatePersonalEvent = async (req, res) => {
  ...
  try {
    ...
    if (isPastDate && !pastDateConfirmed) {
      return res.status(400).json({ message: 'Cette date est passée, confirmez pour modifier l’événement.' });
    }
