...
router.get('/:matricule/sections', async (req, res) => {
  ...
  try {
    const [sections] = await db.query(`
      SELECT DISTINCT s.ID_section, s.niveau, s.nom_section, sp.nom_specialite, d.Nom_departement, f.nom_faculte
      FROM Section s
...
// Route pour récupérer les étudiants d'une section spécifique (sans annee_inscription)
...
// Route pour exporter les étudiants d'une section en JSON (pour Excel)
router.get('/:matricule/section/:sectionId/excel', async (req, res) => {
  const { matricule, sectionId } = req.params;
  ...
  try {
    ...
    const exportData = students.map(student => ({
      ...
      Nom: student.nom || 'Non défini',
      Prénom: student.prenom || 'Non défini',
      État: student.etat || 'Non défini',
      Groupe: student.num_groupe ? `Groupe ${student.num_groupe}` : 'Non assigné',
    }));

    res.status(200).json(exportData);
  } catch (err) {
    console.error('Erreur lors de la récupération des données pour Excel:', err);
    res.status(500).json({ error: 'Erreur serveur.' });
  }
});
...
router.get('/:matricule/section/:sectionId/pdf', async (req, res) => {
  ...
  try {
    const [section] = await db.query(`
      SELECT s.niveau, s.nom_section, sp.nom_specialite, d.Nom_departement, f.nom_faculte
      FROM Section s
      JOIN Enseignant_Section es ON s.ID_section = es.ID_section
      JOIN Specialite sp ON s.ID_specialite = sp.ID_specialite
      JOIN Departement d ON sp.ID_departement = d.ID_departement
    ...
    doc
      .fontSize(12)
      .text(`${section[0].niveau} - ${section[0].nom_specialite} - ${section[0].nom_section}`, { align: 'center' });
    doc
      .fontSize(10)
